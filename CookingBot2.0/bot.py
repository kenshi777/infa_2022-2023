...
req_breakfast = requests.get(
        f"https://www.themealdb.com/api/json/v1/1/filter.php?c=Breakfast"
    ).json()

req_dessert = requests.get(
        f"https://www.themealdb.com/api/json/v1/1/filter.php?c=Dessert"
    ).json()
...
for i in range(len(req_breakfast['meals'])):
    list_of_breakfast_meal.append(req_breakfast['meals'][i]['strMeal'])
list_of_dessert_meal = []
for i in range(len(req_dessert['meals'])):
    list_of_dessert_meal.append(req_dessert['meals'][i]['strMeal'])
...
print(get_meal_from_category('Pasta', 1))
...
async def breakfast(message: types.Message):
    buttons = [InlineKeyboardButton(list_of_breakfast_meal[i], callback_data=f"breakfast_{list_of_breakfast_meal[i]}") for i in range(len(list_of_breakfast_meal))]
...
async def dessert(message: types.Message):
    ...
    buttons = [InlineKeyboardButton(list_of_dessert_meal[i], callback_data=f"dessert_{get_list[i]}") for i in range(len(get_list))]
    inline_keyboard = InlineKeyboardMarkup()
    for j in range(len(buttons)):
        inline_keyboard.add(buttons[j])
